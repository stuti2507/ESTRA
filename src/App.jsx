...
export default function App() {
  ...
  const filteredInsights = useMemo(
    () => insights.filter(
      (item) => (filter.discipline === 'All' || item.tags.discipline === filter.discipline)
  ...
  const publishInsight = (event) => {
    ...
    setStatusMessage('Insight published.');
  };
  ...
  const toggleSaved = (id) => {
    setSavedInsightIds((prev) => (prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]));
  };

  const toggleFollowTag = (tag) => {
    setFollowedTags((prev) => (prev.includes(tag) ? prev.filter((x) => x !== tag) : [...prev, tag]));
  };
  ...
  const approveApplication = (id, nextStatus) => {
    setApplications((prev) => prev.map((x) => (x.id === id ? { ...x, status: nextStatus } : x)));
  };
    ...
    return (
      <div className="section" style={{ minHeight: '100vh' }}>
        <div className="section-inner">
          ...
          <h2 className="section-title">ESTRA Dashboard</h2>
          <p className="section-body">Signed in as: {session?.user?.email || 'Unknown user'}</p>
          <div className="publish-form">
            <p>Status: {statusMessage}</p>
            <button className="btn-solid" onClick={() => setWorkspaceOpen(false)}>Return to Landing</button>
            <button
              className="btn-ghost"
              onClick={async () => {
    ...
    <>
      <nav>
        ...
        <div className="nav-links">
          <a href="#why-now">Structural Context</a>
          <a href="#ecosystem">System Diagram</a>
      ...
      <section className="hero" id="hero">
        <div className="hero-inner">
          ...
          <div className="hero-actions">
            <button className="btn-solid" onClick={() => window.location.assign('#apply')}>Apply to Join</button>
            <button className="btn-ghost" onClick={() => setAuthOpen(true)}>{session ? 'Account' : 'Sign up / Login'}</button>
            {session && <button className="btn-ghost" onClick={() => setWorkspaceOpen(true)}>Open Workspace</button>}
          </div>
        </div>
      </section>
      ...
      <section className="section" id="insights">
        <div className="section-inner">
          ...
          <div className="tag-row">
            {[...disciplineTags, ...regionTags, ...formatTags].map((tag) => (
              <button key={tag} className={`eco-tag ${followedTags.includes(tag) ? 'active-tag' : ''}`} onClick={() => toggleFollowTag(tag)}>Follow {tag}</button>
            ))}
          </div>
          ...
          {isApprovedMember && (
            <form className="publish-form" onSubmit={publishInsight}>
              ...
              <div className="filters">
                <select name="discipline" required><option value="">Discipline</option>{disciplineTags.map((x) => <option key={x}>{x}</option>)}</select>
                <select name="region" required><option value="">Region</option>{regionTags.map((x) => <option key={x}>{x}</option>)}</select>
                <select name="format" required><option value="">Format</option>{formatTags.map((x) => <option key={x}>{x}</option>)}</select>
              </div>
      ...
      <footer>
        ...
        <div className="footer-copy">{statusMessage}</div>
        <button className="btn-ghost" onClick={() => setAuthOpen(true)}>{session ? 'Switch account' : 'Login'}</button>
      </footer>
